...
rodada_diaria <- function(modelo, inicializacao, precipitacao, evapotranspiracao, Emarg, saidaAnterior = matrix(nrow = 0, ncol = 14), dataD = NULL){
  ...
  matrizSaida[1,14] <- RsoloInic / modelo$str #Eq.19 Manual

  if (precipitacao > modelo$ai){
    matrizSaida[1,6] <- ((precipitacao - modelo$ai) ^ 2) / (precipitacao - modelo$ai + (modelo$str - RsoloInic)) #Eq.11
  }else{
    matrizSaida[1,6] <- 0
  }

  if((precipitacao - matrizSaida[1,6]) > evapotranspiracao){
    matrizSaida[1,7] <- evapotranspiracao
  }else{
    matrizSaida[1,7] <- (precipitacao-matrizSaida[1,6]) + (evapotranspiracao - (precipitacao - matrizSaida[1,6])) * matrizSaida[1,14]  #Eq.12
  }

  Capc_tmp <- (modelo$capc / 100) * modelo$str

  if(RsoloInic > Capc_tmp){
    matrizSaida[1,8] <- (modelo$crec / 100) * matrizSaida[1,14] * (RsoloInic - Capc_tmp) #Eq.13
  }else{
    matrizSaida[1,8] <- 0
  }
  ...
  if (RsupInic > modelo$h){
    matrizSaida[1,9] <- (RsupInic - modelo$h) * (1 - K_1ts) #Eq.14
  }else{
    matrizSaida[1,9] <- 0
  }

  matrizSaida[1,10] <- min(c(RsupInic - matrizSaida[1,9], modelo$h1)) * (1 - K_2ts) #Eq.15

  matrizSaida[1,11] <- Rsup2Inic * (1 - K_3ts) #Eq.17

  matrizSaida[1,12] <- max(c(RsupInic - modelo$h1 - matrizSaida[1,9], 0)) * (1 - K_2t2s) #Eq.16

  matrizSaida[1,13] <- RsubInic * (1 - K_kts) #Eq.18

  #Calculo das variaveis de estado ----
  matrizSaida[1,2] <- min(c((RsoloInic + precipitacao - matrizSaida[1,6] - matrizSaida[1,7] - matrizSaida[1,8]), modelo$str))
  matrizSaida[1,5] <- RsubInic + matrizSaida[1,8] - matrizSaida[1,13]
  Rsup_tmp <- ((RsoloInic + precipitacao - matrizSaida[1,6] - matrizSaida[1,7] - matrizSaida[1,8]) - modelo$str)
  matrizSaida[1,3] <- RsupInic + matrizSaida[1,6] - matrizSaida[1,9] - matrizSaida[1,10] - matrizSaida[1,12] + max(c(0, Rsup_tmp))
  matrizSaida[1,4] <- max(Rsup2Inic + matrizSaida[1,9] - matrizSaida[1,11] - Emarg, 0)

  #Calculo da vazão ----
  matrizSaida[1,1] <- (matrizSaida[1,10] + matrizSaida[1,11] + matrizSaida[1,12] + matrizSaida[1,13]) * attributes(modelo)$area / 86.4
